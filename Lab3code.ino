              ...
              if (motorsEnabled) {                                            // run motors only if enabled
                    switch(driveIndex) {                                      // cycle through drive states
                       ...
                       case 1: // Drive forward — motors spin in opposite directions as they are opposed by 180 degrees
                          encoder[0].pos = 0;
                          targetCounts = 50 * cCountsRev / (4.2 * 3.14159);
                          while (encoder[0].pos < targetCounts) {
                             setMotor(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                             setMotor(-1, driveSpeed, cIN1Chan[1], cIN2Chan[1]);  // right motor reverse (opposite orientation)
                          }
                       ...
                       case 2: // Drive backward — motors spin in opposite directions as they are opposed by 180 degrees
                          encoder[0].pos = 0;
                          targetCounts = 25 * cCountsRev / (4.2 * 3.14159);
                          while (-1*encoder[0].pos < targetCounts) {
                          ...
                          driveIndex = 3;                                     // next state: stop for 4 seconds
                          break;


                       case 3: // pause for 4 seconds
                          
                          if (millis() - pauseStart > 4000) { // Check if 4 seconds have elapsed
                             driveIndex = 4; // next state: drive backwards 25 cm
                           }
                          break;


                       case 4: // drive backwards 25 cm
                          encoder[0].pos = 0;
                          targetCounts = 25 * cCountsRev / (4.2 * 3.14159);
                          while (-1*encoder[0].pos < targetCounts) {
                            setMotor(-1, driveSpeed, cIN1Chan[0], cIN2Chan[0]); // left motor reverse 
                          ...
                          driveIndex = 5;                                     // next state: stop for 2 seconds
                          break;


                       case 5: // 2 second pause
                          
                          if (millis() - pauseStart > 4000) { // Check if 2 seconds have elapsed
                               driveIndex = 0; // next state: back to start
                           }
                          break;
                    }
